...
declare module CSemVerPlayground.CSemVersion {
    class CSemVersion implements IEquatable<CSemVersion>, IComparable<CSemVersion> {
        static fromVersionParts(tag: string, major: number, minor: number, patch: number, preReleaseName: string, preReleaseNameIdx: number, preReleaseNumber: number, preReleaseFix: number, marker: string, kind: ReleaseTagKind): CSemVersion;
        static fromDecimal(d: BigJsLibrary.BigJS): CSemVersion;
        static fromFailedParsing(tag: string, isMalformed: boolean, errorMessage: string): CSemVersion;
        major: number;
        minor: number;
        patch: number;
